...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // IMPORTANT: Ensure this URL matches your server.js port and route
    ...
    try {
      ...
      if (response.ok) {
        setStatusMessage({ text: data.msg || 'Message sent successfully!', type: 'success' });
        setFormData({ name: '', email: '', message: '' }); // Reset form
      } else {
        setStatusMessage({ text: data.msg || 'Server Error: Failed to send message.', type: 'error' });
      }
    } catch (error) {
      console.error('Submission Error:', error);
      setStatusMessage({
        text: 'Failed to connect to the backend server (Node.js). Ensure it is running on port 5001.',
        type: 'error'
      });
    } finally {
      setLoading(false);
